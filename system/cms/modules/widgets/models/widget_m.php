...
class Widget_m extends MY_Model
{
	...
	public function insert_instance($input)
	{
		...
		$last_widget = $this->db
			...
			->order_by('`order`', 'desc')
			->limit(1)
			->get_where($this->config->item( 'cms_db_prefix' ).'widget_instances', array('widget_area_id' => $input['widget_area_id']))
			->row();

		$order = isset($last_widget->order) ? $last_widget->order + 1 : 1;

		return $this->db->insert($this->config->item( 'cms_db_prefix' ).'widget_instances', array(
			'title'				=> $input['title'],
	...
	public function update_instance($instance_id, $input)
	{
		...
		return $this->db->update($this->config->item( 'cms_db_prefix' ).'widget_instances', array(
        	'title'				=> $input['title'],
			'widget_area_id'	=> $input['widget_area_id'],
			'options'			=> $input['options']
		));
	...
	function delete_widget($slug)
	{
		...
		if (isset($widget->id))
		{
			$this->db->delete($this->config->item( 'cms_db_prefix' ).'widget_instances', array('widget_id' => $widget->id));
		}
	...
	public function delete_area($id = 0)
	{
		...
		$this->db->delete($this->config->item( 'cms_db_prefix' ).'widget_instances', array('widget_area_id' => $id));
	...
	{
		return $this->db->delete($this->config->item( 'cms_db_prefix' ).'widget_instances', array('id' => $id));
	}
}
